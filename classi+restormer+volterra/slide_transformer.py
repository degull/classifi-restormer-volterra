...
class SlideAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        x = (attn @ v).transpose(1, 2).reshape(B, H, W, C).permute(0, 3, 1, 2)  # (B, C, H, W)
...
class DeformedShifting(nn.Module):
    ...
    def forward(self, x, mode="train"):
        if mode == "train":
            ...
            return shifted_x1 + shifted_x2
        else:
            merged_weight = self.predefined_shift.weight + self.learnable_shift.weight
            optimized_shift = nn.Conv2d(x.size(1), x.size(1), kernel_size=self.kernel_size,
                                        padding=self.kernel_size//2, groups=x.size(1), bias=False)
...
class SlideTransformer(nn.Module):
    def __init__(self, img_size=224, num_classes=25, embed_dim=96, num_heads=6, mlp_ratio=4):
        super().__init__()
    ...
    def forward(self, x, mode="train"):
        ...
        x = self.deformed_shifting(x, mode=mode)
        x = x.flatten(2).transpose(1, 2)  # (B, N, C)
        x = self.attn_layer(x)
        x = self.mlp(x.mean(dim=1))       # (B, C)
        x = self.global_transformer(x.unsqueeze(1))  # (B, 1, C)
        x = x.squeeze(1)                  # (B, C)
...
if __name__ == "__main__":
    model = SlideTransformer(img_size=224, num_classes=25)
