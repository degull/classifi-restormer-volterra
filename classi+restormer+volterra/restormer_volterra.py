...
class RestormerVolterra(nn.Module):
    def __init__(self, in_channels=3, out_channels=3, cond_dim=25, dim=48, num_blocks=[4,6,6,8],
    ...
    def forward(self, x, condition_vector):
        ...
        cond_feat = self.cond_encoder(condition_vector).view(B, -1, 1, 1)
        cond_feat = cond_feat.expand(-1, -1, H, W)

        x1 = self.patch_embed(x) + cond_feat
