...
# ───────────── 경로 설정 ─────────────
CHECKPOINT_PATH     = r"E:/restormer+volterra/checkpoints/restormer_volterra_rain100l/epoch_100.pth"
...
RESULT_SAVE_DIR     = r"E:/restormer+volterra/results/rain100L"
...
# ───────────── 테스트 이미지 목록 ─────────────
img_list = sorted(os.listdir(RAIN100L_TEST_RAIN))

total_psnr, total_ssim = 0.0, 0.0

for filename in tqdm(img_list, desc="🔍 Testing on Rain100L"):
    rain_path = os.path.join(RAIN100L_TEST_RAIN, filename)
    gt_path   = os.path.join(RAIN100L_TEST_GT, filename)
