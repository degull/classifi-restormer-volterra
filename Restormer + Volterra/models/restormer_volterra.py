...
class MDTA(nn.Module):
    def __init__(self, dim, num_heads, bias):
        ...
        self.dwconv = nn.Conv2d(dim * 3, dim * 3, 3, 1, 1, groups=dim * 3, bias=bias)
...
class TransformerBlock(nn.Module):
    def __init__(self, dim, num_heads, ffn_expansion_factor, bias,
                 ...
                 use_volterra_mdta=True, use_volterra_gdfn=True):  # ✅ 추가
        super().__init__()
        ...
        self.norm1 = LayerNorm(dim, bias=bias) if LayerNorm_type == 'WithBias' else BiasFreeLayerNorm(dim)
        self.attn = MDTA(dim, num_heads, bias)
        self.volterra1 = VolterraLayer2D(dim, dim, rank=volterra_rank) if use_volterra_mdta else nn.Identity()

        self.norm2 = LayerNorm(dim, bias=bias) if LayerNorm_type == 'WithBias' else BiasFreeLayerNorm(dim)
        self.ffn = GDFN(dim, ffn_expansion_factor, bias)
        self.volterra2 = VolterraLayer2D(dim, dim, rank=volterra_rank) if use_volterra_gdfn else nn.Identity()
...
class RestormerVolterra(nn.Module):
    def __init__(self, in_channels=3, out_channels=3, dim=48,
        ...
        self.encoder1 = Encoder(dim, num_blocks[0], num_heads=heads[0],
            ...
            LayerNorm_type=LayerNorm_type, volterra_rank=volterra_rank,
            use_volterra_mdta=use_volterra_mdta, use_volterra_gdfn=use_volterra_gdfn)
        self.down1 = Downsample(dim)
...
if __name__ == '__main__':
    model = RestormerVolterra(use_volterra_mdta=True, use_volterra_gdfn=False)
