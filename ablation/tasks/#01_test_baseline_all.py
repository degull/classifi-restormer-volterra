# E:/restormer+volterra/ablation/tasks/test_baseline_all.py
import sys, os, torch
...
from re_dataset.sidd_dataset import SIDD_Dataset   # ✅ 올바른 클래스
from re_dataset.csd_dataset import CSDDataset

# ---------------- Config ----------------
DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")
CKPT = r"E:\restormer+volterra\ablation\checkpoints\train_baseline\baseline_epoch26_psnr25.95_ssim0.8010.pth"
...
def evaluate(model, loader, max_eval=None):
    ...
    with torch.no_grad():
        for i, (inp, gt) in enumerate(tqdm(loader, leave=False)):
            if max_eval and i >= max_eval:  # ⚡ 빠르게 평가
                break
...
if __name__ == "__main__":
    ...
    for name, dataset in DATASETS.items():
        ...
        psnr, ssim = evaluate(model, loader, max_eval=50)  # ⚡ 최대 50장만 평가
        results[name] = (psnr, ssim)
        print(f"[RESULT] {name}: PSNR={psnr:.2f}, SSIM={ssim:.4f}")

    # ✅ 최종 요약
    print("\n==== Final Results (Baseline Transformer Only) ====")
    for name, (psnr, ssim) in results.items():
        print(f"{name:10s} | PSNR={psnr:.2f} | SSIM={ssim:.4f}")


""" ==== Final Results (Baseline Transformer Only) ====
Rain100H   | PSNR=25.74 | SSIM=0.7868
Rain100L   | PSNR=27.62 | SSIM=0.8882
GoPro      | PSNR=23.16 | SSIM=0.8228
SIDD       | PSNR=28.62 | SSIM=0.8814
CSD        | PSNR=16.36 | SSIM=0.6837 """